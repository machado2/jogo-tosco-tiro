...
pub fn spawn_ship_visual(
    ...
    let width_units_base: f32 = match choice {
        0 => 1.0,
        1 => 0.8,
        2 => 0.8,
        3 => 1.0,
    ...
    root.with_children(|g| {
        ...
        let hull_mat = materials.add(ColorMaterial {
            color: base_color,
            ..default()
        });
        ...
        let wing_mat = materials.add(ColorMaterial {
        ...
        for i in 0..4 {
            ...
            let wing = meshes.add(Mesh::from(Rectangle {
        ...
        let cockpit = meshes.add(Mesh::from(Rectangle {
            half_size: Vec2::new(0.07, 0.06),
            }));
        let cockpit_mat = materials.add(ColorMaterial {
            color: Color::rgba(0.6, 1.6, 2.0, 1.0),
            ..default()
        });
        g.spawn(MaterialMesh2dBundle {
        ...
        let stripe = meshes.add(Mesh::from(Rectangle {
            half_size: Vec2::new(0.4, 0.01),
            }));
        let stripe_mat = materials.add(ColorMaterial {
            color: Color::rgba(2.0, 1.4, 0.5, 0.6),
            ..default()
        });
        g.spawn(MaterialMesh2dBundle {
        ...
        let antenna = meshes.add(Mesh::from(Rectangle {
            half_size: Vec2::new(0.01, 0.08),
            }));
        let antenna_mat = materials.add(ColorMaterial {
            color: base_color * 1.2,
            ..default()
        });
        g.spawn(MaterialMesh2dBundle {
    ...
    ShipBounds {
        width_units,
        height_units: height_units_base,
    }
}
...
pub fn spawn_engine_glow(
    ...
    let mesh = Mesh::from(Rectangle {
        half_size: Vec2::new(0.5, 0.15),
        });
    let mesh_h = meshes.add(mesh);
    let mat_h = materials.add(ColorMaterial {
        color: Color::rgba(1.8, 0.9, 0.4, 0.9),
        ..default()
    });
    parent.spawn((

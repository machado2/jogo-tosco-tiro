use crate::core::{GamePhase, Muted, Score};
use crate::gameplay::components::{Charge, Health};
...
fn ui_system(
    ...
    // Top-left HUD
    egui::Window::new(egui::RichText::new("HUD").strong())
        .title_bar(false)
        ...
        .show(ctx, |ui| {
            ...
            ui.small(format!(
                "Cursor: {:.0},{:.0}",
                cursor.screen.x, cursor.screen.y
            ));
    ...
    match state.get() {
        GamePhase::Paused => {
            egui::Area::new("paused")
                .fixed_pos(center_text_pos(ctx, "PAUSED"))
                .show(ctx, |ui| {
        ...
        GamePhase::GameOver => {
            egui::Area::new("game_over")
                .fixed_pos(center_text_pos(ctx, "GAME OVER"))
                .show(ctx, |ui| {
...
fn progress_bar(ui: &mut egui::Ui, label: &str, frac: f32, color: egui::Color32) {
    ...
    ui.painter()
        .rect_filled(rect, 3.0, egui::Color32::from_black_alpha(64));
    ...
    if label == "Health" {
        ...
        let _glow_alpha = (30.0 + (ui.ctx().input(|i| i.time) * 3.0).sin() * 15.0) as u8;
        ui.painter().rect_stroke(
            fill,
            3.0,
            egui::Stroke::new(
                2.0,
                egui::Color32::from_rgb(bar_color.r(), bar_color.g(), bar_color.b())
                    .linear_multiply(1.2)
                    .gamma_multiply(1.3),
            ),
        );
        ...
        if frac >= 0.99 {
            ...
            ui.painter()
                .rect_stroke(fill, 3.0, egui::Stroke::new(3.0, glow_color));
            ...
            ui.painter().rect_stroke(
                inner_rect,
                2.0,
                egui::Stroke::new(1.5, egui::Color32::from_rgb(200, 230, 255)),
            );
        }
    } else {
        ui.painter().rect_filled(fill, 3.0, color);

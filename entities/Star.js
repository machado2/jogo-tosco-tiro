class Star extends Entity {
    ...
    update() { if (temporizes(10)) { if (gameState.score > 5000 && temporizes(50)) this.pulse(); if (++this.y > SCREEN_HEIGHT) this.destroy(); } this.updateMeshPosition(); this.updateBodyFromEntity(); }
    pulse() { for (let a = 0; a < Math.PI * 2; a += 0.05) enemyEntities.push(new Missile(this.x, this.y, Math.cos(a) * 10, Math.sin(a) * 10, false)); }
    onDestroy() { gameState.score += 100; this.pulse(); try { audioSystem.playPowerup(); } catch {} }
}
